...
$uploadDir = ROOT_PATH . '/uploads/leadership';
if (!is_dir($uploadDir)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add' || $action === 'edit') {
        ...
        if ($action === 'add') {
            $sql = "INSERT INTO leadership (full_name, position, email, phone, biography, photo, status, order_index) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$full_name, $position, $email, $phone, $biography, $photo_path, $status, $order_index]);
        } else {
            // Update existing
            if ($photo_path) {
                $sql = "UPDATE leadership SET full_name = ?, position = ?, email = ?, phone = ?, biography = ?, photo = ?, status = ?, order_index = ? WHERE id = ?";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([$full_name, $position, $email, $phone, $biography, $photo_path, $status, $order_index, $id]);
            } else {
                $sql = "UPDATE leadership SET full_name = ?, position = ?, email = ?, phone = ?, biography = ?, status = ?, order_index = ? WHERE id = ?";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([$full_name, $position, $email, $phone, $biography, $status, $order_index, $id]);
            }
        }
    } elseif ($action === 'delete') {
        $id = intval($_POST['id']);
        $pdo->prepare("DELETE FROM leadership WHERE id = ?")->execute([$id]);
    }
...
$leadership = $pdo->query("SELECT * FROM leadership ORDER BY order_index ASC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Manage Leadership - NTUPROBUDCAM Admin</title>
...
<body>
    <div class="admin-layout">
        <aside class="sidebar">
            ...
            <ul class="sidebar-nav">
                ...
                <li><a href="leadership.php" class="active"><i class="fas fa-users"></i> Leadership</a></li>
        ...
        <main class="main-content">
            <div class="d-flex justify-between align-center mb-4">
                ...
                <button onclick="showModal()" class="btn btn-primary"><i class="fas fa-plus mr-2"></i>Add Leadership Member</button>
            </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?= $person['status'] ?></td>
                            <td>
                                <button onclick="editModal(<?= htmlspecialchars(json_encode($person)) ?>)" class="btn btn-sm" style="background: var(--primary-blue); color: white; margin-right: 0.5rem;">Edit</button>
    ...
    <div id="leadershipModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center;">
        <div style="background: white; border-radius: var(--radius-lg); padding: 2rem; max-width: 600px; width: 90%; max-height: 90vh; overflow-y: auto;">
            <h2 style="margin-bottom: 1.5rem;" id="modalTitle">Add Leadership Member</h2>
            <form method="POST" enctype="multipart/form-data" id="leadershipForm">
                <input type="hidden" name="action" value="add" id="formAction">
                <input type="hidden" name="id" value="" id="formId">
                <div class="form-group">
                    <label class="form-label">Full Name</label>
                    <input type="text" class="form-control" name="full_name" id="formFullName" required>
                </div>
                <div class="form-group">
                    <label class="form-label">Position</label>
                    <input type="text" class="form-control" name="position" id="formPosition" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="biography" id="formBiography" rows="3"></textarea>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <small class="text-muted">Leave empty to keep existing photo when editing</small>
                </div>
    ...
    <script>
        function showModal() {
            document.getElementById('modalTitle').textContent = 'Add Leadership Member';
            ...
            document.getElementById('formBiography').value = '';
        ...
        function editModal(person) {
            document.getElementById('modalTitle').textContent = 'Edit Leadership Member';
            document.getElementById('formAction').value = 'edit';
            document.getElementById('formId').value = person.id;
            document.getElementById('formFullName').value = person.full_name;
            document.getElementById('formPosition').value = person.position;
            document.getElementById('formEmail').value = person.email || '';
            document.getElementById('formPhone').value = person.phone || '';
            document.getElementById('formBiography').value = person.biography || '';
            document.getElementById('formStatus').value = person.status;
            document.getElementById('formOrderIndex').value = person.order_index;
            document.getElementById('leadershipModal').style.display = 'flex';
        }
        
        function hideModal() {
            document.getElementById('leadershipModal').style.display = 'none';
        }
    </script>
</body>
</html>
